...
> **멀티-에이전트 읽기 순서**: `personal/P###_이름/hot.md` (본인 폴더, 있으면 먼저) → `wiki/hot.md` → `wiki/index.md` → 관련 페이지.
> 개인 최신 컨텍스트를 공유 지식베이스보다 우선 반영한다.
...
1. If `personal/P###_이름/hot.md` exists (본인 폴더), read it first. If it answers the question, respond immediately.
2. Read `wiki/hot.md`. If it answers the question, respond immediately.
3. If not, read `wiki/index.md`. Scan descriptions for the answer.
...
1. **Read** `personal/P###_이름/hot.md` first (본인 폴더, if exists). Contains your most recent personal context.
2. **Read** `wiki/hot.md`. It may already have the answer or directly relevant context.
