...
> **멀티-에이전트 안전 규칙**:
> - **참가자**: dry-run 전용. 리포트 생성은 가능하지만 자동 수정(auto-fix)은 절대 수행하지 않음.
> - **Vault Keeper**: auto-fix 허용. CLAUDE.md에서 vault-keeper 역할 확인 후 수행.
>
> 참가자가 auto-fix를 요청하면 "이 작업은 Vault Keeper 전용입니다. Vault Keeper에게 요청하세요."라고 응답.
